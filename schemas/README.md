...
Run `npm run research:records -- validate-identity <path>`, `validate-event <path>`, `validate-outcome <path>`, `validate-evaluation-spec <path>`, `validate-source <path>`, `validate-fact <path>`, `validate-fact-snapshot <path>`, `validate-calendar-snapshot <path>`, `validate-market-observations <path>`, `validate-market-capture <path>`, or `validate-company-ledger <path>` for dependency-free repository validation. Run `score-events track-record/event-forecast-ledger.csv` for ledger validation, candidate/forecast/outcome ancestry checks, source/fact cutoff checks, calendar-session selection, committed market-observation/capture reconciliation, deterministic target resolution, and scoring. The validator enforces publication gates and cross-field date rules that JSON Schema alone cannot express.
